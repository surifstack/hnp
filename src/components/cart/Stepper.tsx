...
export function Stepper({ current, verified }: { current: Step; verified: boolean }) {
  const steps = [
    { n: 1, label: "Details" },
    { n: 2, label: "Verify" },
    { n: 3, label: "Address" },
  ...
  return (
    <div className="flex items-center justify-between bg-white/90 rounded-full px-3 py-2 border-2 border-black shadow">
      {steps.map((s, i) => {
        const done = s.n < current || (s.n === 2 && verified && current > 2);
        ...
        return (
          <div key={s.n} className="flex items-center flex-1 last:flex-none">
            <div className="flex flex-col items-center gap-0.5">
              ...
              <span className="text-[9px] uppercase font-bold tracking-wide">{s.label}</span>
            </div>
            {i < steps.length - 1 && (
              <div className={`flex-1 h-0.5 mx-1 ${s.n < current ? "bg-black" : "bg-black/20"}`} />
            )}
          </div>
        );
      })}
