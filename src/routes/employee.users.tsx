...
export const Route = createFileRoute("/employee/users")({
  head: () => ({ meta: [{ title: "Employee Users — MININOTE" }] }),
  component: EmployeeUsersPage,
});

function EmployeeUsersPage() {
  const users = useEmployeeStore((s) => s.users);
  const toggleUserStatus = useEmployeeStore((s) => s.toggleUserStatus);
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl border-2 border-black bg-white p-5 shadow-md">
        ...
        <p className="mt-1 text-sm text-muted-foreground">Employee user moderation (dummy).</p>
      </div>
